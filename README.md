...
7. **重排序**：对检索结果进行关键词或语义重排序，提升相关性
8. **答案生成**：基于检索到的上下文生成答案
...
系统采用智能分块策略，确保文档块的大小适中且保持上下文完整性：
...
1. **段落分割**: 按双换行符分割文本为段落
2. **句子分割**: 对过长的段落按句号、问号、感叹号分割
3. **块合并**: 将过短的块与相邻块合并
4. **添加重叠**: 在块之间添加重叠内容，保持上下文连续性
5. **元数据标注**: 为每个块添加来源文档、块索引等元数据

### 分块优势

- **上下文保持**: 通过重叠机制确保语义连续性
- **灵活调整**: 可根据文档类型调整分块参数
- **元数据丰富**: 每个块包含来源和位置信息，便于追溯

## 指标监控

系统集成了 Micrometer + Prometheus 进行指标监控，可通过 `/actuator/prometheus` 端点获取指标数据：
